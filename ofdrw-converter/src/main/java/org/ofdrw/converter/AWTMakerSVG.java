 ...
 */
public abstract class AWTMakerSVG {
     ...
     * OFD内部使用毫米作为基本单位
     * <p>
     * 如果需要更加精确的表示单位请使用 {@link #AWTMakerSVG(org.ofdrw.reader.OFDReader, double)}
     *
     * @param reader OFD解析器
     * @param ppm    每毫米像素数量(Pixels per millimeter)
     */
    public AWTMakerSVG(OFDReader reader, int ppm) {
        this.reader = reader;
     ...
     * @author iandjava
     */
    public AWTMakerSVG(OFDReader reader, double ppm) {
        this.reader = reader;
        this.resourceManage = reader.getResMgt();
        this.pages = reader.getPageList();
        if (this.ppm > 0) {
    ...
    private void writePath(Graphics2D graphics, PathObject pathObject, List<CT_DrawParam> drawParams, Matrix parentMatrix) {
     ...
     //   Color xiugai =new Color(255,255,255);
    ...
    private void writeText(Graphics2D graphics, TextObject textObject, List<CT_DrawParam> drawParams, Matrix parentMatrix) {
        ...
        for (TextCode textCode : textObject.getTextCodes()) {
          ...
          //  content = content.replace("此件由咸阳市电子证照库于2021年11月25日16时01分14秒生成，仅供全国建筑市场监管公共服务平台数据共享使用", "水印");
         //   content = content.replace(" ", "水印");
